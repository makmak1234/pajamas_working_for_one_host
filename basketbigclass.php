...
//header("Location: http://127.0.0.1/Pajamas/basketbig.php");
		...
		//print_r('this->dirthan: ');
		//print_r($this->dirthan); print_r('<br>');
...
END;
	...
	function basketbigclear1(){
		...
		if(isset($_GET["idclear1"])){
			//print_r('isset_idclear: ');
		//print_r($_GET["idclear1"]); print_r('<br>');

			$idclear = $this->foo_mysgli->sanitizeMySQL($_GET["idclear1"]);
			//print_r('idclear: ');
			//print_r($idclear); print_r('<br>');
			$id = $idclear;
			$k = $this->foo_mysgli->sanitizeMySQL($_GET["kgoods1"]);
			$this->idarr = $_SESSION["idbasketsmall"];
			$this->nid = $_SESSION["nid"];
			//print_r('idarr: ');
